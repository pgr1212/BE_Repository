...
import com.example.kwu_graduation.domain.requirements.hakbun23.dto.CheckStatus;
...
import com.example.kwu_graduation.domain.requirements.hakbun23.spec.GraduationRequirement;
import com.example.kwu_graduation.domain.requirements.hakbun23.spec.RequirementSpecLoader;
...
/**
 * 2023학번 모듈이 실제 요건 JSON을 읽어 판정까지 동작하는지 확인하는 스모크 테스트.
...
class RequirementCheckSmokeTest {
    ...
    /** 균형교양 영역에 걸친 10과목 30학점(23학번 균형 21/18학점 요건을 충분히 초과) */
    ...
    @Test
    @DisplayName("2023 정융 - 총학점·전공 60·균형교양 21학점이 충족으로 판정된다")
    void jeongyung() {
        GraduationRequirement spec = loader.load(2023, Department.JEONGYUNG);
        List<KlasSubjectGradeResponse> subjects = new ArrayList<>(balance30());
        subjects.add(culture("졸업작품"));

        RequirementCheckResponse res = checker.check(spec, credits(133, 60, 22), subjects,
    ...
    @Test
    @DisplayName("2023 소프트 - TOPCIT·세부전공·공학프로그램 분기가 동작한다")
    void software() {
        ...
        RequirementCheckResponse res = checker.check(spec, credits(133, 60, 19), subjects,
